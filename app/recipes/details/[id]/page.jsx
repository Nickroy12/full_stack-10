...
import ReportButton from "@/ui/ReportButton"; // ১. নতুন ইম্পোর্ট করা হলো
...
const DetailsPage = async ({ params }) => {
  ...
  const initialHasLiked =
    recipe?.likedBy && user?.id ? recipe.likedBy.includes(user.id) : false;
  ...
  const recipeStatus = recipe.status?.toLowerCase() || "usual";
            ...
            <div>
              ...
              <div className="flex flex-row justify-between gap-3 p-3 bg-gray-50 dark:bg-gray-900/50 rounded-xl">
                ...
                <div className="flex-1 flex flex-col items-center justify-center last:border-0">
                  ...
                  <span className="text-base font-black text-green-600 dark:text-green-400 capitalize">
                    {recipe.difficulty}
                  </span>
                </div>
              </div>
...
<form action={'/api/payment'} method="POST">
  ...
  <button type="submit" className="flex-[2] inline-flex items-center justify-center gap-2 px-6 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold text-sm transition shadow-md shadow-green-600/20 dark:shadow-none">
    Get Premium Recipe
  </button>
</form>

              <ReportButton recipeId={id} currentStatus={recipeStatus} />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
          ...
          <div className="w-full md:w-2/3 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col">
            <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4 border-b-2 border-green-600 pb-2 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-green-600"></span> How to
              Cook
            </h2>
              ...
              {recipe.instructions?.split("\n").map((step, index) => {
                if (step.startsWith("Step")) {
                  return (
                    <h3
                      key={index}
                      className="text-sm font-bold text-green-600 dark:text-green-400 pt-3 first:pt-0 flex items-center gap-1"
                    >
                      {step}
                    </h3>
                  );
                ...
                return (
                  <p
                    key={index}
                    className="pl-4 border-l-2 border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400"
                  >
                    {recipe.instructions}
                  </p>
                );
